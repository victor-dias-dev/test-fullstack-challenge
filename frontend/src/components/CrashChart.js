...
function getMultiplierColor(multiplier, phase) {
    if (phase === "CRASHED")
        return "#ff3366";
    if (multiplier >= 5)
        return "#00ff88";
...
export function CrashChart() {
    ...
    function drawMultiplierText(ctx, W, H, multiplier, phase) {
    ...
    return (_jsx("canvas", { ref: canvasRef, width: 800, height: 320, className: "w-full rounded-xl border border-[#1e1e2e]", style: { background: "#0a0a0f" } }));
}
